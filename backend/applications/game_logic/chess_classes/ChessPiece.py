...
class Piece(object):
    ...
    def is_in_danger(self, src_c, src_l):
        ...
        enemy_side = 'black' if self.side.name == 'white' else 'white'
        enemy_pieces = self.board.get_side_pieces(enemy_side)
        # print 'Piece.is_in_danger: ennemy_side:%s' % ennemy_side
        for enemy_piece in enemy_pieces:
            # print 'Piece.is_in_danger: ennemy_piece:%s' % ennemy_piece
            enemy_piece_c, enemy_piece_l = self.board.get_piece_coords(enemy_piece)
            if enemy_piece_c and enemy_piece_l:
                # print 'Piece.is_in_danger: ennemy %s (c:%s, l:%s)' % (ennemy_piece, ennemy_piece_c, ennemy_piece_l)
                if enemy_piece.is_move_valid(enemy_piece_c, enemy_piece_l, src_c, src_l):
                    print ('Piece.is_in_danger: %s is targeted by %s' % (self, enemy_piece))
                    return True
